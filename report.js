(function () {
  ...
  app.innerHTML = `<section class="panel report-panel" id="reportCapture"><header class="report-header"><p class="eyebrow">心理测试工具</p><h1>我的测评结果</h1><p>生成时间：${new Date().toLocaleString("zh-CN",{hour12:false})}</p></header><section class="report-block"><h2>来访者资料</h2>${facts}</section><section class="report-block"><h2>已使用量表与评测结果</h2><p class="report-count">${completionText}</p><div class="report-results">${items}</div></section><section class="report-block" data-html2canvas-ignore="true"><h2>给机构留言（可选）</h2><p class="muted-copy">如有补充情况、咨询诉求或对测评结果的疑问，可在这里留言，机构管理方和系统管理员可在后台查看。</p><textarea class="note-textarea" id="clientMessage" rows="4" placeholder="可选填写"></textarea><p id="messageState" class="form-message"></p><button class="secondary-btn" id="saveMessageBtn" type="button">保存留言</button></section><div class="actions" data-html2canvas-ignore="true"><button class="secondary-btn" id="clearReportBtn">清空本次数据</button><button class="primary-btn" id="saveReportBtn">截图保存结果</button></div></section>`;
