(async function () {
  ...
  if (!intake && !localResults.length) {
    app.innerHTML = '<section class="panel empty-report"><p class="eyebrow">咨询报告</p><h1>本次还没有资料</h1><p class="lead">请先填写来访者信息并完成测评。</p><a class="primary-btn button-link" href="intake.html">填写来访者信息</a></section>';
    return;
  }

  const sessions = buildSessions();
  const allResults = sessions.flatMap(session => session.results || []);
  const completionText = allResults.length ? `共记录 ${sessions.length} 次测评批次，${allResults.length} 项测评结果` : "尚未完成测评";
  ...
  app.innerHTML = `<section class="panel report-panel" id="reportCapture"><header class="report-header"><p class="eyebrow">心理测试工具</p><h1>我的测评结果</h1><p>所属机构：${escapeHTML(intake?.organizationName || selfProfile?.organizationName || "系统直属")} · 生成时间：${fmt(new Date().toISOString())}</p></header><section class="report-block"><h2>来访者资料</h2>${facts}</section><section class="report-block"><h2>既往所有测评结果</h2><p class="report-count">${escapeHTML(completionText)}</p><div class="report-results">${sessionBlocks}</div></section><section class="report-block" data-html2canvas-ignore="true"><h2>给机构留言（可选）</h2><p class="muted-copy">如有补充情况、咨询诉求或对测评结果的疑问，可在这里留言，机构管理方和系统管理员可在后台查看。</p><textarea class="note-textarea" id="clientMessage" rows="4" placeholder="可选填写"></textarea><p id="messageState" class="form-message"></p><button class="secondary-btn" id="saveMessageBtn" type="button">保存留言</button></section><div class="actions" data-html2canvas-ignore="true"><button class="secondary-btn" id="clearReportBtn">清空本次数据</button><button class="primary-btn" id="saveReportBtn">截图保存结果</button></div></section>`;
  ...
  document.getElementById("saveReportBtn").addEventListener("click", async () => {
    ...
    link.download=`心理测试工具-咨询报告-${new Date().toISOString().slice(0,10)}.png`;
  ...
  function renderSession(session, index) {
    ...
    return `<article class="scale-group"><h3>${escapeHTML(title)}</h3><p class="muted-copy">本批次共 ${session.results.length} 项测评结果。</p>${session.results.map((item, i) => renderResult(item, i)).join("")}</article>`;
  }
