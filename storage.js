(function () {
  ...
  function friendlyError(response,payload){const msg=String(payload?.msg||payload?.message||"");if(response.status===401||response.status===403)return new Error("登录已失效或没有管理权限。");if(msg.includes("Signup requires a valid password")||msg.includes("valid password"))return new Error("请先设置至少 8 位密码，再发送验证码。");if(msg.includes("invalid_invite"))return new Error("机构代码无效或已停用，请核对后重试。");if(msg.includes("organization_code_limit"))return new Error("每个机构最多只能设置 3 组机构代码。");if(msg.includes("organization_inactive"))return new Error("机构账号已暂停、到期或尚未开通。");if(payload?.code==="23505")return new Error("该机构代码已存在。");return new Error(msg||"云端连接失败，请稍后重试。");}
