(function () {
  ...
  function friendlyError(response,payload){if(response.status===401||response.status===403)return new Error("登录已失效或没有管理权限。");if(payload?.message?.includes("invalid_invite"))return new Error("机构代码无效或已停用，请核对后重试。");if(payload?.code==="23505")return new Error("该机构代码已存在。");return new Error(payload?.msg||payload?.message||"云端连接失败，请稍后重试。");}
