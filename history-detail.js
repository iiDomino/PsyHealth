(async function () {
  ...
  app.innerHTML=`<section class="panel report-panel"><header class="report-header"><p class="eyebrow">来访者详情</p><h1>${esc(i.name)}的测试结果</h1><p>咨询时间：${new Date(record.createdAt).toLocaleString("zh-CN",{hour12:false})}</p></header><section class="report-block"><h2>来访者信息</h2>${facts}</section><section class="report-block"><h2>详细结果</h2><p class="report-count">已完成 ${results.length} 项测评</p><div class="report-results">${items}</div></section></section>`;
})();
